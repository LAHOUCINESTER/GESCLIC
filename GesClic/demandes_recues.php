        ...
        <tbody>
            ...
            if ($role == 'employeur') {
                while ($row = mysqli_fetch_assoc($rslt)) {
                        ...
                        ?>
                        <td><?php echo $row['N_dmd']; ?></td>
                        <td><?php echo $row['DateSoumission']; ?></td>
                        <td><?php echo $row['nom_employe']; ?></td>
                        <td><?php echo $row['type_dmd']; ?></td>
                        <td><?php echo $row['etat']; ?></td>
                        <td><?php echo $row['Statut']; ?></td>
                        <td><button class="btn btn-link btn-details" data-request-id="<?php echo $row['N_dmd']; ?>"><i class="bi bi-eye-fill"></i></button></td>
                        <td><?php echo $row['vu']; ?></td>
                        <td><button type="button" class="btn btn-primary btn-reply" data-request-id="<?php echo $row['N_dmd']; ?>" data-toggle="modal" data-target="#answerModal">Répondre</button></td>
                    </tr>
                <?php
                } 
                ...
                while ($row = mysqli_fetch_assoc($res_rec)) {
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-primary btn-reply " data-request-id="<?php echo $row['N_dmd']; ?>" data-toggle="modal" data-target="#answerModal"> Répondre </button>
                        </td>
                        <td><?php echo $row['vu']; ?></td>
    ...
    btnMarkConsulted.forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            fetch('vu_dmd.php?id=' + requestId)
                ...
                .then(data => {
                    if (data === 'success') {
                        row.cells[7].textContent = 'Oui'; // Mise à jour du texte de la colonne "Déjà consulté"
                    }
                });
        });
    });
